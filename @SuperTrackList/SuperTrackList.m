classdef SuperTrackList < handle
    %SuperTrackList is a class defining the List holding the Tracks detected in the SuperSlice, for the PTV Point Matching Algorithm
    %
    ...
    %                Labeling.
    %
    %Properties: cannyThreshold (default = 0.3)
    %            AngleLimit (default = 90)
    %            ImageLength = (default: set from length of slice image)
    %            PreferenceOfPositionOverOrientation (default = 3)
    %
    %CONSTRUCTORS:
    %   superTrackListInstance = SteroTrackList() - Create empty object of StreoTrackList Class
    %
    %   superTrackListInstance = SteroTrackList(sTLInstanceOld) - Initialize from another STL Instance <sTLInstanceOld>
    %
    %   superTrackListInstance = SteroTrackList(superSliceIn) - Initialize by detecting tracks in SuperSlice <steroSliceIn>
    %
    ...
    %
    %METHODS:
    %   sTLInstance = STL.Match() - Match tracks over the current List and return the matching tracks in a new list <sTLInstance>
    %       **In case of several cameras - matching is between all consecutive pairs (1-2, 2-3, 3-4, ...).
    %           Then, each pair is stored in a row of the STL cell array
    %           **ATTENTION: Inverting images where necessary...
    %
    ...
    properties (SetAccess = public, GetAccess = public)
        ...
        AngleLimit = 90;
        ImageLength = 128;
        PreferenceOfPositionOverOrientation = 128/48;
    end %Public Properties
    
    methods
        %CONSTRUCTORS
        function superTrackListInstance = SuperTrackList(varargin)
            %   superTrackListInstance = SuperTrackList() - Create empty object of StreoTrackList Class
            %
            %   superTrackListInstance = SuperTrackList(sTLInstanceOld) - Initialize from another STL Instance <sTLInstanceOld>
            %
            %   superTrackListInstance = SuperTrackList(superSliceIn) - Initialize by detecting tracks in SuperSlice <steroSliceIn>, using
            %       default canny Threshold
            %
            %   stereoTrackListInstance = StereoTrackList(stereoSliceIn, cannyThresholdIn) - Initialize by detecting tracks in StereoSlice <steroSliceIn>, using
            %       default canny Threshold
            ...
            if (nargin<1)
                superTrackListInstance.data = [];
                return
            elseif (nargin == 1)
                
                if strcmpi(class(varargin{1}) ,'SuperTrackList') %Initialize from another STL
                    
                    nCamPairs = size(varargin{1},3);
                    
                    superTrackListInstance(nCamPairs) = SuperTrackList();
                    for iterCamPairs = 1:nCamPairs
                        superTrackListInstance.data = varargin{1}.data;
                        superTrackListInstance.cannyThreshold = varargin{1}.cannyThreshold;
                        superTrackListInstance.AngleLimit = varargin{1}.AngleLimit;
                        superTrackListInstance.ImageLength = varargin{1}.ImageLength;
                        superTrackListInstance.PreferenceOfPositionOverOrientation = varargin{1}.PreferenceOfPositionOverOrientation;
                        return;
                    end
...
%                     superTrackListInstance.ImageLength = max(size(sliceIn)); %Length of the image {assumption - nCameraViews << length(image) }
            
                    for iterCamView = 1:size(sliceIn,3)
                        superTrackListInstance.data{1,iterCamView} = superTrackListInstance.detectTracksInSlice(sliceIn(:,:,iterCamView));
                    end
        ...
        function STLInstance = Match(self)
            ...
            %
            %Match tracks over the current List and return the matching tracks in a new list <sTLInstance>
            %
            %**In case of several cameras - matching is between all consecutive pairs (1-2, 2-3, 3-4, ...).
            %           Then, each pair is stored in a row of the STL cell array, and the second direction has only two columns (FOR CURRENT MATCH)
            %           **ATTENTION: Inverting images where necessary...
            ...
            STLInstance = SuperTrackList(); %Initialize Array (for more than two camera views)
